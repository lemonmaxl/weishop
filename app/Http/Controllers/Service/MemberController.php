...
class MemberController extends Controller
{
    ...
    if($phone != '') {
	    ...
	    if($tempPhone->code == $phone_code) {
	        ...
	        $member = new Member;
	        $member->phone = $phone;
	        $member->password = md5('ws' + $password);
	        $member->save();
    ...
    // 邮箱注册
	        ...
	        if($validate_code == '' || strlen($validate_code) != 4) {
      		...
      		$m3_email->to = $email;
      		$m3_email->cc = 'walter@doweit.com';
      		$m3_email->subject = '沃特博客验证';
      		$m3_email->content = '请与24小时点击该链接完成验证.http://weishop.com/service/validate_email' . '?member_id=' . $member->id . '&code=' . $uuid;
      		...
      		Mail::send('email_register', ['m3_email' => $m3_email], function ($m) use ($m3_email) {
            	//$m->from('' , '');
            	$m->to($m3_email->to, '尊敬的用户')->subject($m3_email->subject);
        	});
	...
	public function login(Request $request){
		...
		if ($member == null) {
			...
			if(md5('ws'+$password) != $member->password){
				$m3_result->status = 3;
